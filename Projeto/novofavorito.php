...
//coloca imagem pequena na pagina
        ...
        $tpl_content = file_get_contents('TPL/novofavorito.tpl');
        ...
        if($t == 'up'){
            $sql = 'SELECT id FROM favoritos_uploads WHERE id_utilizador = '.$_SESSION['utilizador_id'].' AND id_upload = '.$idr.' LIMIT 1';
        }else{
            $sql = 'SELECT id FROM favoritos_utilizadores WHERE id_utilizador = '.$_SESSION['utilizador_id'].' AND id_favorito = '.$idr.' LIMIT 1';
        }
        $query = mysqli_query($link_bd, $sql);
        ...
        if($query->num_rows > 0){
            ...
            if($t == 'up'){
                $_SESSION['mensagem_erro'] = 'Este projeto/trabalho já está nos seus favoritos.';
                $extra = 'trabalho.php?id='.$idr;
            }else{
                $_SESSION['mensagem_erro'] = 'Este utilizador já está nos seus favoritos.';
                $extra = 'perfil.php?id='.$idr;
            }
            header ("location: http://$host$uri/$extra");
            exit;
        }
        ...
        if($query->num_rows == 0){
            $_SESSION['mensagem_erro'] = 'Um parametro inválido foi inserido ao adicionar favorito.';
            $host = $_SERVER['HTTP_HOST'];
            $uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
            $extra = 'index.php';
            header ("location: http://$host$uri/$extra");
