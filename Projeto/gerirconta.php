<?php
       ...
       // $tpl_homeblock1 = file_get_contents('TPL/homeblock1.tpl');
        ...
        $tpl_content = file_get_contents('TPL/gerirconta.tpl');
        
        if (!isset($_SESSION['utilizador_id']) || !isset($_SESSION['utilizador_username'])){
            
            
            //envia-o de volta para o index
            $_SESSION['mensagem_erro'] = 'É necessário efetuar o Login para poder gerir a conta.';
...
//coloca imagem pequena na pagina
            ...
            //PASSAGEM PARA A IMAGEM MEDIA DE GERIRCONTA.PHP---------------
            //-----------------------------
            //------------------------------
                          //encontra imagem do utilizador----------------
             $imagempath = ''; //caminho da imagem
             $path = 'imagens_utilizadores/'; //caminho da pasta
             $foi = 0; //confirmaçao que tem imagem
             //verifica se o utilizador ja tem uma imagem media
             $filename = $path.$_SESSION['utilizador_id'].'_media.jpg';
             if (!file_exists($filename)) { //se não existir
                ...
                if($foi == 1){
                    ...
                    $imagem = WideImage::loadFromFile($imagempath);
                   // Redimensiona a imagem
                    $imagem = $imagem->resize(495, 371);
                        // Guarda a imagem
                    $pathcompleto = $path.$_SESSION['utilizador_id'].'_media.jpg'; // ex: 5_pequena, 82_pequena
                    $imagem->saveToFile($pathcompleto, 40); // Coloca a imagem media no disco
               }else{
                    $pathcompleto = $path.'imagem_media.jpg';     
                     }
               
             }else{
                 $pathcompleto = $path.$_SESSION['utilizador_id'].'_media.jpg';
             }
                
             //----------------------------------------------
            
//coloca imagem media na pagina
            $htmlimg = '<img src="'.$pathcompleto.'" alt="">';
            $tpl_content = str_replace("()-imagem_conta-()", $htmlimg, $tpl_content);
             //-----------------
            //coloca descricao antiga no formulario
            $sql = 'SELECT descricao, email FROM utilizadores WHERE id = '.$_SESSION['utilizador_id'].' LIMIT 1';
            $query = mysqli_query($link_bd, $sql);
            if(!$query){
                die("query falhou em gerir conta email,descr");
            }
            while($r = mysqli_fetch_assoc($query)){
                $descricao_ant = $r['descricao'];
                $email_ant = $r['email'];
            }
            $tpl_content = str_replace("()-descricaop-()", $descricao_ant, $tpl_content);
            $tpl_content = str_replace("()-emailp-()", $email_ant, $tpl_content);
            //--------------
        ...
        //IMAGEM(ANTIGA) É COLOCADA PARA VER
