...
//apresenta menus
    ...
    if(isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0){
        ...
        $_SESSION['mensagemadmin'] = "Esse upload não existe.";
        $host = $_SERVER['HTTP_HOST'];
        $uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
    ...
    //pesquisa para saber se existe
    $sql = "SELECT titulo FROM uploads WHERE id = $idupload LIMIT 1";
    ...
    //pesquisa SQL para saber o total das denuncias a este upload
    $sql = "SELECT count(*) FROM reports_uploads WHERE id_upload = ".$idupload;
    $query = mysqli_query($link_bd, $sql);
    if(!$query){
    ...
    $sql = "SELECT * FROM reports_uploads WHERE id_upload = $idupload ORDER BY datahora DESC LIMIT $ant_primeira, 10";
    $query = mysqli_query($link_bd, $sql);
    if(!$query){
    ...
    $i = 0;
    $tplconteudo = '<p> <h3>-----------------------------------------<p>VER DENÙNCIAS<br><br>Upload nº'.$idupload.'</p>-----------------------------------------</h3><br/>';
    while($i < 10){
        ...
        if($query->num_rows > $i){
            ...
            if($_SESSION['administrador_rank'] > 1){
                $tplconteudo .= '<p><strong>| </strong>'.$datahora.' <strong>|<br>| Utilizador que denunciou: </strong>'.$username_denunciante.' <strong>|<br>| Título: </strong>'.$titulo.' <strong>|<br>| Conteúdo:</strong> '.$conteudo.' <br><a href="apagarreport.php?t=up&id='.$id_report.'" onclick="return confirmar_apagarnoticia(); return FALSE;">Apagar</a><br></p>';
            }else{
    ...
    if($idpag > 1){
        $paganterior = $idpag - 1;
        $tplconteudo .= '<a href="reportsupload.php?id='.$idupload.'&p='.$paganterior.'">Anterior </a>';
    }
    
    if($numeropaginas > $idpag){
       $pagproxima = $idpag + 1;
        $tplconteudo .= '<a href="reportsupload.php?id='.$idupload.'&p='.$pagproxima.'">Próxima</a>';  
    }
