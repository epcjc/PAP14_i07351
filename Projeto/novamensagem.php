...
$_SESSION["pagina"] = $_SERVER['PHP_SELF'];
       ...
       // $tpl_homeblock1 = file_get_contents('TPL/homeblock1.tpl');
        ...
        if (!isset($_SESSION['utilizador_id']) || !isset($_SESSION['utilizador_username'])){
            ...
            $_SESSION['mensagem_erro'] = 'É necessário efetuar o Login para ter acesso a esta página.';
...
//coloca imagem pequena na pagina
        ...
        $tpl_content = file_get_contents('TPL/novamensagem.tpl');
        //definiçao
        if(isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0){
            $iddestin = $_GET['id'];
            $sql = "SELECT username FROM utilizadores WHERE id=$iddestin LIMIT 1";
            $query = mysqli_query($link_bd, $sql);
            if(!$query) die("nao deu para pesquisar o destinatario");
            if($query->num_rows > 0){
                $r = mysqli_fetch_assoc($query);
                $destinatario = htmlspecialchars($r['username']);
            }else{
                $destinatario = '';
            }
            $tpl_content = str_replace('()-destinatario-()', $destinatario, $tpl_content);
        }else{
            $tpl_content = str_replace('()-destinatario-()', '', $tpl_content);
        }
        //acaba definiçao
