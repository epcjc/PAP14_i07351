...
//coloca imagem pequena na pagina
        ...
        $tpl_content = file_get_contents('TPL/novoreport.tpl');
        ...
        if($t == ''){
            $_SESSION['mensagem_erro'] = 'Parametro inválido em reportar.';
            $host = $_SERVER['HTTP_HOST'];
            $uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
            $extra = 'index.php';
            header ("location: http://$host$uri/$extra");
        ...
        if(isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0){
            ...
            $_SESSION['mensagem_erro'] = 'Foi inserido um parametro inválido em reportar.';
            $host = $_SERVER['HTTP_HOST'];
            $uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
            $extra = 'index.php';
            header ("location: http://$host$uri/$extra");
        ...
        $query = mysqli_query($link_bd, $sql);
        if(!$query) die("nao deu pesquisa existe nvreport");
        if($query->num_rows == 0){
            $_SESSION['mensagem_erro'] = 'Um parametro inválido foi inserido em reportar.';
            $host = $_SERVER['HTTP_HOST'];
            $uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
            $extra = 'index.php';
            header ("location: http://$host$uri/$extra");
